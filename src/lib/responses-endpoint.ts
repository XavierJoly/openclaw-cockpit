 ...
 * Extracted from gateway/route.ts to avoid invalid route exports in Next.js 16.
 */
...
export function ensureResponsesEndpoint(): void {
  ...
  _responsesSetupPromise = (async () => {
    try {
      ...
      const gw = (root as Record<string, unknown>)?.gateway as
      ...
      const responses = endpoints?.responses as
        | Record<string, unknown>
        | undefined;
